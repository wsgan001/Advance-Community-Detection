...
for i,val in H.nodes(data=True):
   ...
   # H.node[i]['candidate'] =node_list['candidate'][c]
   # H.node[i]['candidate_confidence'] =node_list['candidate_confidence'][c]
#    H.node[i]['relevant_yn'] =node_list['relevant_yn'][c]
#    H.node[i]['relevant_yn_confidence'] =node_list['relevant_yn_confidence'][c]
    H.node[i]['sentiment'] =node_list['sentiment'][c]
    H.node[i]['senticonfidence'] =node_list['sentiment_confidence'][c]
#    H.node[i]['subject_matter'] =node_list['subject_matter'][c]
#    H.node[i]['subject_matter_confidence'] =node_list['subject_matter_confidence'][c]
 #   H.node[i]['name'] =node_list['name'][c]
  #  H.node[i]['retweet_count'] =node_list['retweet_count'][c]
#    H.node[i]['text'] =node_list['text'][c]
...
for cl in attr_index_class.keys():
    ...
    if H.degree(item) >= k_core[1] and 'class' in H.node[item].keys():
        ...
        print len(k.edges())
        print k.edges()
        nx.write_edgelist(k,'subgraph_edgelist.txt',delimiter =',',data=False)
